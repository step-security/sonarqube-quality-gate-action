...
@test "fail when GITHUB_OUTPUT is unset (set-output fallback removed for security)" {
  # The deprecated ::set-output workflow command is vulnerable to log-injection,
  # so the set_output helper no longer falls back to it. Modern runners always
  # provide GITHUB_OUTPUT; if it's missing the script exits non-zero rather
  # than silently emitting a deprecated command.
  export SONAR_TOKEN="test"
  unset GITHUB_OUTPUT
  echo "serverUrl=http://localhost:9000" >> metadata_tmp
  echo "ceTaskUrl=http://localhost:9000/api/ce/task?id=AXlCe3gsFwOUsY8YKHTn" >> metadata_tmp
